...
class SnakeGame:
	def __init__(self, w=200, h=200, isDisplayed=True):
		self.w = w
		self.h = h
		self.isDisplayed = isDisplayed
		self.gen = None
		self.path_to_destionation = None
	...
	def play_step(self, action):
		...
		# BFS
		self.path_to_destionation = p_bfs.bfs(self.body, self.h, self.w, self.head.y, self.head.x, self.food.y, self.food.x)
...
if __name__ == "__main__":
	snake = SnakeGame(w=700, h=500)
	while True:
		new_state, reward, game_over, score = snake.play_step(0);
		if game_over:
			break
